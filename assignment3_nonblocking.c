...
int **array_ghostcells(int **a, int rm, int cm, int rank, int size, MPI_Request *req)
{
	//one-dimensional data distribution occurs in this method
	//send and receive upper and lower rows
  MPI_Isend(&a[1][1], cm, MPI_INT, (rank-1+size)%size, 0, MPI_COMM_WORLD, &req[0]);
  MPI_Isend(&a[rm][1], cm, MPI_INT, (rank+1)%size, 0, MPI_COMM_WORLD, &req[1]);
  MPI_Irecv(&a[0][1], cm, MPI_INT, (rank-1+size)%size, 0, MPI_COMM_WORLD, &req[2]);
  MPI_Irecv(&a[rm+1][1], cm, MPI_INT, (rank+1)%size, 0, MPI_COMM_WORLD, &req[3]);

	//send and receive left and right columns
  MPI_Isend(&a[1][1], 1, MPI_INT, (rank-1+size)%size, 1, MPI_COMM_WORLD, &req[4]);
  MPI_Isend(&a[1][cm], 1, MPI_INT, (rank+1)%size, 1, MPI_COMM_WORLD, &req[5]);
  MPI_Irecv(&a[1][0], 1, MPI_INT, (rank-1+size)%size, 1, MPI_COMM_WORLD, &req[6]);
  MPI_Irecv(&a[1][cm+1], 1, MPI_INT, (rank+1)%size, 1, MPI_COMM_WORLD, &req[7]);
}
...
int main(int argc, char *argv[])
{
  ...
  for(int i=1; i<local_row+1; i++)
	...
	array_ghostcells(aptr, local_row, local_col, rank, size, requests);
  ...
  while (gencount <= generations)
  {
    ...
    for (int i = 1; i < local_row + 1; i++)
    {
  	...
  	//exchange boundary data for the next generation
		array_ghostcells(bptr, local_row, local_col, rank, size, requests);
    MPI_Waitall(8, requests, MPI_STATUSES_IGNORE);
